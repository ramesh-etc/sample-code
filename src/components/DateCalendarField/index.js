...
const DateCalendarField = (props) => {
    const { input, label, autoFocus, disabled, required, labelStyle, placeholder, customLabelClass, fieldHeight, disableFuture, minDate, maxDate, meta: { touched, error, warning } } = props;
        ...
        <Grid container sx={{
            ...
            <Grid item xs={12} className={classes.formControl}>
                {label ? <InputLabel
                    ...
                    htmlFor={name}
                >
                    <Typography className={customLabelClass || classes.label} sx={labelStyle}>{label}&nbsp;{props?.helperText ? touched && ((error && <span className={classes.error}>{error}</span>) || (warning && <span>{warning}</span>)) : null}</Typography>
